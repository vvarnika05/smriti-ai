...
const Button = ({
  ...
  const baseClasses =
    "inline-flex items-center justify-center rounded-full font-semibold transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 disabled:opacity-50 disabled:pointer-events-none transform hover:scale-105";
  const variants = {
    default:
      "bg-gradient-to-r from-primary to-[#9dff07] hover:from-[#9dff07] hover:to-primary text-black hover:shadow-xl hover:shadow-primary/30",
    outline:
...
const Badge = ({
  ...
  children: React.ReactNode;
  variant?: "default" | "primary" | "secondary"; // adjust variants as needed
  className?: string;
}) => {
...
const TableRow: React.FC<CardProps> = ({ children, className = "" }) => (
  <tr
...
const TableHead: React.FC<CardProps> = ({ children, className = "" }) => (
  <th
    className={`h-12 px-4 text-left align-middle font-semibold text-gray-300 ${className}`}
  >
    {children}
  </th>
);

const TableCell: React.FC<CardProps> = ({ children, className = "" }) => (
  <td className={`p-4 align-middle text-white ${className}`}>{children}</td>
);
      ...
      try {
        ...
        if (!contributorsResponse.ok) {
          console.error(
            ...
            contributorsResponse.statusText
          );
        ...
        try {
          ...
          if (lastUpdatedResponse.ok) {
            ...
            console.warn(
              "LastUpdated fetch failed:",
              lastUpdatedResponse.status,
              lastUpdatedResponse.statusText
            );
          ...
          <div className="mb-16">
            <Card className="backdrop-blur-md">
              <CardHeader>
                ...
                <CardDescription>
                  Complete list of all contributors ranked by their
                  contributions
                </CardDescription>
              </CardHeader>
              <CardContent>
